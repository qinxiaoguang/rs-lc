pub struct Solution {}
/*
 * @lc app=leetcode.cn id=950 lang=rust
 *
 * [950] 按递增顺序显示卡牌
 *
...
// @lc code=start
impl Solution {
    // 倒推题，或者模拟题，但是怎么倒推是个问题
    pub fn deck_revealed_increasing(deck: Vec<i32>) -> Vec<i32> {
        // 把正确结果的各个下标放入队列中来进行模拟,还能这样模拟，真是长见识了
        let mut deque: Vec<i32> = (0..deck.len() as i32).collect();
        let mut res = vec![0; deck.len()];
        let mut deck = deck;
        deck.sort();
        for n in deck {
            res[deque.remove(0) as usize] = n;
            if !deque.is_empty() {
                let p = deque.remove(0);
                deque.push(p);
            }
        }
        res
